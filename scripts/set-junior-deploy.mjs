/* Set junior override, commit, and push to main.
   
   Usage:
     npm run set-junior:deploy Andy
     npm run set-junior:deploy Blair
*/
...
if (!args.length) {
  console.error('Usage: npm run set-junior:deploy <first-name>');
  console.error('Example: npm run set-junior:deploy Andy');
  process.exit(1);
}
...
try {
  ...
  // Stage, commit, push
  try {
    execSync('git add data/junior-override.json', { cwd: join(__dirname, '..'), stdio: 'pipe' });
    console.log('✓ Staged override');

    execSync('git commit -m "Set junior tracker override" -m "Holder: ' + fullName + '" -m "Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"', {
      cwd: join(__dirname, '..'),
      stdio: 'pipe'
    });
